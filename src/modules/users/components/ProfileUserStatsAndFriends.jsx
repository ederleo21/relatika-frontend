...
export const ProfileUserStatsAndFriends = ({ user }) => {
  ...
  const { followers_count, following_count, featured_friends } = user;

  const handleModalList = (label) => {
    setLabelStat(label)
    setIsOpen(true)
  }
  ...
  return (
    <>
      <div className="flex-1 flex flex-col space-y-2 min-w-0">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-6 gap-4 bg-gray-50 rounded-xl p-4 shadow-sm min-w-0">
              {[
                ...
                { value: followers_count, label: "Seguidores", details: true},
                { value: following_count, label: "Siguiendo", details: true},
          ...
          <div className="bg-gray-50 rounded-xl shadow p-4 mt-4">
            ...
            <div className="flex space-x-4 overflow-x-auto p-2">
              {featured_friends.map(friend => (
                <Link key={friend.id} to={`/user/${friend.id}/`}>
                  <div className="flex-shrink-0 w-32 bg-white rounded-xl shadow p-3 flex flex-col items-center text-center hover:shadow-lg hover:bg-gray-50 hover:ring-2 hover:ring-indigo_light"
                  >
                    <img
                      src={friend.avatar}
                      alt={friend.username}
      ...
      {isOpen && (
        <FollowUserList title={labelStat} labelStat={labelStat} isOpen={isOpen} onClose={() => setIsOpen(false)} userId={user.id} />
      )}
    </>
  )
}