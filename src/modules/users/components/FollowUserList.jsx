    ...
    return (
        <ModalContent onClose={onClose}>
          {loading ? (
              ...
              <ul className="flex flex-col gap-3 w-full">
                {listUsers.map(user => (
                  <Link key={user.id} to={`/user/${user.id}/`} className="w-full">
                    <li className="flex items-center gap-4 py-3 px-4 bg-white rounded-xl shadow-md hover:shadow-lg hover:bg-gray-50 transition cursor-pointer">
                      <img
                        src={user.avatar}
                        alt={user.username}
                      ...
                      <div className="flex flex-col">
                        <span className="text-lg font-semibold text-gray-800">{user.first_name}</span>
            ...
            <p className="text-center text-gray-400 text-lg mt-6">No hay usuarios para mostrar.</p>
          )}
        </ModalContent>
    )
}
