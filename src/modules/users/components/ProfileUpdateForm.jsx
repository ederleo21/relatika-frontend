        ...
        <Form className='space-y-4'>
            <ModalForm isOpen={isOpen} onClose={onClose} title={title} isSubmitting={isSubmitting}>    
              <div className="grid grid-cols-2 gap-x-4 gap-y-3 font-poppins">
                  ...
                  <InputField name="bio" placeholder="Bio" label="Bio" className='col-span-2' as='textarea'/>
                  <InputField name="birth_date" label="Fecha de nacimiento" type='date'/>
                  <div className="col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-1">Avatar</label>
                    {values.avatar ? (
                      <img src={URL.createObjectURL(values.avatar)} alt="Nuevo avatar" className="w-16 h-16 rounded-full mb-2"/>
                    ) : (
                      authUser.avatar && (
